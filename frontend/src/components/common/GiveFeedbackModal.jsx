...
return axiosSecure.patch(`/feedbacks/${feedbackId}`, data);      // console.log(data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["feedbacks", courseId]);
      resetTheModal();
  ...
  const onSubmit = (data) => {
  // data already contains { feedback: "..." } because of register("feedback")
  ...
  return (
    ...
    (existingFeedbacks && existingFeedbacks.length > 0) ? onUpdate : onSubmit
  )}
          className="space-y-4"
        >
          <div>
            ...
            <textarea
  {...register("feedback", { required: true })} // Change "description" to "feedback"
  defaultValue={existingFeedbacks?.[0]?.feedback || ""} // Match the field name here too
  rows={4}
  className="w-full ..."
