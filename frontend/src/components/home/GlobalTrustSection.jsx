            ...
            <div className="flex lg:flex-col gap-3 mb-4">
              {testimonials.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setIndex(i)}
                  className={`transition-all duration-500 rounded-full cursor-pointer ${index === i ? 'w-12 h-1.5 lg:w-1.5 lg:h-12 bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]' : 'w-2 h-1.5 lg:w-1.5 lg:h-2 bg-gray-200 hover:bg-gray-300'
                    }`}
                />
              ))}
            </div>
